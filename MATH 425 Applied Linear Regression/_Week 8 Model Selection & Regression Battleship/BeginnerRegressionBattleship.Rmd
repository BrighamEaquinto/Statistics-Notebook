...
```{r}
plot(y ~ x4, data = rbdata, col = as.factor(x5))
b <- coef(mylm)
curve(b[1] + b[2]*x, add = TRUE, col = "skyblue") # base line
curve( (b[1] + b[2]*x) # base line (change line)
      + b[3]*x^3, add = TRUE, col = "green") # change line 

# pairs(rbdata)
# pairs(rbdata, panel = panel.smooth, col=alpha(rbdata$X2, 0.5)) # attempt to change alpha
# pairs(rbdata, panel = panel.smooth, col = as.factor(rbdata$X2))
...
$$
  Y_i = \beta_0 + \beta1 * X_4 + \epsilon_i \quad \text{where} \ \epsilon_i \sim N(0, \sigma^2)
$$
